...
export default function VariantPreview({
  ...
  const [variantPreview, setVariantPreview] = React.useState<
    ProductVariant[] | null
  >();
  ...
  //giả sử thay đổi tiền tệ của variant
  const handleUpdateVariant = (id: number, price: number) => {
    const currentVariants = getValues("variants");
    console.log("Current variants,:", id, currentVariants, price);
    const updatedVariants = currentVariants.map((variant: ProductVariant) =>
      variant.id === id ? { ...variant, price: price } : variant
    );
    setVariantPreview(updatedVariants)
    setValue("variants", updatedVariants);

    console.log("sau khi nhấn cập nhật", getValues("variants"));
  };
      ...
      <div className="w-full md:w-1/2">
        ...
        {selectedVariant && (
          <div className="space-y-2">
            ...
            <button
              type="button"
              onClick={() => handleUpdateVariant(selectedVariant.id, 40000)}
            >
              Cập nhật
            </button>
          </div>
